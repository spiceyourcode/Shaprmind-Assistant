      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3">
            ...
            <span>{call.caller_name || call.caller_number}</span>
            <span className={cn(
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground border-b border-border pb-3">
          ...
          {call.sentiment_avg !== undefined && (
            <span className="flex items-center gap-1"><SentimentDot sentiment={call.sentiment_avg} /> Sentiment: {(call.sentiment_avg * 100).toFixed(0)}%</span>
          )}
        </div>
        ...
        <ScrollArea className="flex-1 -mx-6 px-6">
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-semibold uppercase text-muted-foreground">
                          {msg.role === 'customer' ? 'Customer' : 'AI Rep'}
                        </span>
          ...
          {/* Action Points */}
          {call.action_points.length > 0 && (
            <section className="mb-4">
              <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Action Points</h4>
              <div className="space-y-2">
                {call.action_points.map((ap) => (
                  <div key={ap.id} className="flex items-center gap-3 text-sm p-2 rounded-lg bg-muted/50">
                    {ap.delivery_status === 'success' && <CheckCircle2 className="w-4 h-4 text-success shrink-0" />}
                    {ap.delivery_status === 'failed' && <XCircle className="w-4 h-4 text-destructive shrink-0" />}
                    {ap.delivery_status === 'pending' && <Loader2 className="w-4 h-4 text-warning shrink-0 animate-spin" />}
                    <span className="text-foreground">{ap.description}</span>
                    <span className="ml-auto text-xs text-muted-foreground">
                      {ap.attempts > 0 && `${ap.attempts} attempt${ap.attempts > 1 ? 's' : ''}`}
                    </span>
                  </div>
                ))}
              </div>
