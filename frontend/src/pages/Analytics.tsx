  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard title="Total Calls" value={data.total_calls.toLocaleString()} icon={Phone} trend={{ value: '+8%', positive: true }} />
        <StatsCard title="Avg Sentiment" value={`${(data.avg_sentiment * 100).toFixed(0)}%`} icon={TrendingUp} trend={{ value: '+3%', positive: true }} />
        <StatsCard title="Escalation Rate" value={`${(data.escalation_rate * 100).toFixed(1)}%`} icon={AlertTriangle} trend={{ value: '-2%', positive: true }} />
        <StatsCard title="Avg Handle Time" value={`${Math.floor(data.avg_handle_time / 60)}m ${data.avg_handle_time % 60}s`} icon={Clock} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="glass-card p-5">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={data.call_volume}>
        ...
        <div className="glass-card p-5">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <PieChart>
              <Pie data={data.escalation_reasons} dataKey="count" nameKey="reason" cx="50%" cy="50%" outerRadius={90} label={({ reason, percent }) => `${reason} ${(percent * 100).toFixed(0)}%`}>
                {data.escalation_reasons.map((_, i) => (
        ...
        <div className="glass-card p-5 lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={data.duration_by_day}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
